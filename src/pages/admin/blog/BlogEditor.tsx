...
import { ArrowLeft, Save, Loader2, Globe, Image as ImageIcon, Search, Layout, Settings as SettingsIcon, Eye } from 'lucide-react';
...
const BlogEditor = () => {
    ...
    const [activeTab, setActiveTab] = useState<'content' | 'preview' | 'seo' | 'settings'>('content');
    ...
    const handleContentImageUpload = (url: string) => {
        ...
        // SEO için Alt Text iste
        const altText = window.prompt('Görsel için açıklama (Alt Text - SEO için önemlidir):', 'Görsel Açıklaması');
        const finalAltText = altText || 'Görsel Açıklaması';
    ...
    return (
        <FadeIn>
            <form onSubmit={handleSubmit} className="max-w-5xl mx-auto space-y-8 pb-20">
                {/* Header */}
                <div className="flex items-center justify-between sticky top-0 bg-background/95 backdrop-blur z-30 py-4 border-b border-white/5 px-4 -mx-4 lg:-mx-8 lg:px-8">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="hidden sm:flex bg-secondary rounded-lg p-1 border border-white/10">
                            ...
                            <button
                                ...
                                className={`px-3 py-1.5 text-sm font-medium rounded-md transition-all ${activeTab === 'preview' ? 'bg-white/10 text-white' : 'text-text-muted hover:text-white'}`}
                            >
                                <Eye size={14} className="inline mr-2" /> Önizleme
                            </button>
                        </div>
                ...
                <div className="sm:hidden flex overflow-x-auto gap-2 pb-2 -mx-4 px-4 border-b border-white/5">
                    ...
                    <button type="button" onClick={() => setActiveTab('preview')} className={`whitespace-nowrap px-4 py-2 text-sm font-medium rounded-full border ${activeTab === 'preview' ? 'bg-accent-blue/10 border-accent-blue text-accent-blue' : 'border-white/10 text-text-muted'}`}>Önizleme</button>
                </div>
                        ...
                        <div className="bg-white text-black p-8 rounded-2xl min-h-[500px]">
                            <div className="prose prose-lg max-w-none">
                                ...
                                {formData.cover_image && (
                                    <img src={formData.cover_image} alt={formData.title} className="w-full h-64 object-cover rounded-xl mb-8" />
                                )}
                                <ReactMarkdown
